...
class STUDENT_CONSTANT:
    ...
    def learn(self, teaching_strategy):
        if teaching_strategy == "WSCS_Teach":
            teacher = WSCS_Teach(self.target_amount, 20)
        else:
            teacher = WSCS_Teach(self.target_amount, 20)

        while (True):
            cur_index = teacher.get_next_teach_sample()
            if cur_index == -1:
                break
            ...
            judge = teacher.teach_judge(answer, right_answer)
